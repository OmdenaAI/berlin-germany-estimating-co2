...
try:
    ...
    with open('src/tasks/task-3-dashboard/style.css') as f:
    ...
    if item_row['Agriculture'].iloc[0] == item_row['Food_processing'].iloc[0] == item_row['Transport'].iloc[0] == item_row['Packaging'].iloc[0] == 0:
        ...
        pie_plot = pd.melt(item_row, id_vars=['item'], value_vars=[
                           'Agriculture', 'Food_processing',    'Transport', 'Packaging'], var_name="CO2", value_name="CO2value")
    ...
    fig2 = px.bar(df, x='CO2_per_kg', y='item', color='CO2_per_kg', log_x=True, title='Carbon Footprint Comparison', labels={
                  'CO2_per_kg': 'Carbon Emission per kg', 'item': 'Products'})
